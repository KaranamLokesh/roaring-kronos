...
\begin{figure*}[t]
  ...
  distributions. \emph{Middle:} full $s_1\!\cdot\!s_2$ token frequency
  sorted by rank; the top~49 tokens cover~80\% of all bars (red
  dashed line). \emph{Bottom-left:} $s_1$ run-length distribution
  (consecutive bars sharing the same coarse mood) showing the
  long-tail clustering structure. \emph{Bottom-right:} number of
  distinct~$s_2$ tokens observed per~$s_1$ context;
...
Figure~\ref{fig:dataloader} visualises the per-window rare-token
hit distribution and the token frequency rank curve under each
sampler.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig2_dataloader_benchmark.png}
  \caption{Vanilla uniform vs.\ Roaring stratified
  ($f=0.30$) dataloader.
  \emph{Left:} histogram of the number of rare~(shock) tokens per
  window; Roaring's distribution has a heavier tail.
...
\begin{figure*}[t]
  ...
  Kronos-base (B=32, T=512).
  \emph{Left:} stacked bars showing each phase's contribution to total
  step time; forward and backward together are~$>95\%$ of every step.
  \emph{Right:} side-by-side per-phase comparison. The
  data-loading and H2D-transfer phases (the only ones Roaring affects)
  are barely visible on this scale. End-to-end speedup is
  $0.965\times$, within run-to-run noise.}
  \label{fig:timing}
\end{figure*}
...
trajectories and the per-month RankIC heatmap that makes this
regime asymmetry visible.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_backtest_comparison.png}
  \caption{Three-way backtest comparison (zero-shot,
  vanilla-finetuned, Roaring-finetuned) on the BTC-USD~1h test split.
  \emph{Top:} cumulative return of the $\sigma_{\text{last}}$ signal
  with 0.15\% round-trip cost, against buy-and-hold benchmark
  (black dashed). \emph{Middle:} RankIC and Sharpe by signal type.
  \emph{Bottom:} monthly RankIC heatmap. Green cells indicate
  predictive months, red cells anti-predictive. Roaring-FT's
  April~2026 cell~($+0.195$) is the highest-IC cell of any
  model-month combination in the experiment.}
  \label{fig:backtest}
\end{figure*}
